...
// CredentialProviderContext is derived from authenticated governance context;
// it is never populated from ordinary invocation arguments.
type CredentialProviderContext struct {
	Subject string
	RunID   string
}

// CredentialProvider resolves one administrator-authored binding for the
// governed context. context.Context carries the caller's resolution deadline.
type CredentialProvider interface {
	Resolve(context.Context, domain.CredentialBinding, CredentialProviderContext) (CredentialCapability, error)
}
