...
- [x] DATA-018 Run real-PostgreSQL concurrency/property tests proving tenant isolation, replay safety, conflict semantics, rollback, single attempt ownership, fencing, outbox atomicity, and record `docs/phase-2-evidence.md`.
...
- [x] AUTH-007 Implement the ThinkPixelAG authorization adapter with deadlines, typed request/response validation, stable reason codes, correlation IDs, and no policy duplication inside TG. (2026-08-28)
- [x] AUTH-008 Enforce authorization constraints as narrowing only; prove TG cannot expand allowed repositories/resources/actions/argument bounds returned by AG. (2026-08-28)
...
- [x] AUTH-013 Run cross-tenant/run forgery, stale decision, malformed AG response, AG timeout/outage, revocation-freshness, cache-poisoning, and constraint-expansion tests; record `docs/phase-3-evidence.md`. (2026-08-28)
...
- [x] API-002 Implement JSON Schema input/output validation with bounded compilation/cache behavior, deterministic errors, recursion/size safeguards, and tests for malformed/hostile schemas. — completed 2026-09-02, commit 8eb8a85, evidence: `go test -count=1 ./internal/schema`; `go vet ./internal/schema`; `go test -race -count=1 ./internal/schema`; `go test -count=1 ./...`; dependency source/license/vulnerability checks
- [x] API-003 Implement tool publication validation so a version cannot be published without a connector operation, credential-binding selector, retry semantics, required resource projection, metering semantics, request/result limits, and reviewed descriptions. — completed 2026-09-02, commit 5dba1e9, evidence: `go test -race -count=1 ./internal/domain ./internal/schema`; `go test -count=1 ./...`; `go vet ./...`; scoped `git diff --check`
- [x] API-004 Implement immutable tool-version persistence and reject direct mutation of security-relevant fields after publication; changes require a new version. — completed 2026-09-02, evidence: `go test -race -count=1 ./internal/adapters/postgres ./internal/domain ./internal/canonicaljson`; `go test -count=1 ./...`; `go vet ./...`; integration test compiled (live PostgreSQL unavailable)
- [x] API-005 Implement tenant exposure/visibility rules and authorization-filtered discovery without cross-tenant enumeration leaks. — completed 2026-09-02, evidence: `go test -race -count=1 ./internal/ports ./internal/app ./internal/adapters/postgres`; `go test -count=1 ./...`; `go vet ./...`; integration test compiled (live PostgreSQL unavailable)
- [x] API-006 Implement `GET /v1/tools` with deterministic ordering, cursor pagination, bounded page sizes, authorization filtering, and stable OpenAPI examples. — completed 2026-09-02, evidence: `go test -race -count=1 ./internal/adapters/http ./internal/app ./internal/ports ./internal/adapters/postgres`; `go test -count=1 ./...`; `go vet ./...`; `make openapi-check`; `make lint`
- [x] API-007 Implement `GET /v1/tools/{tool_id}` or equivalent version-aware describe contract with enumeration-safe errors and trusted metadata projection. — completed 2026-09-02, evidence: `go test -race -count=1 ./internal/adapters/http ./internal/app ./internal/ports ./internal/adapters/postgres`; `go test -count=1 ./...`; `go vet ./...`; `make openapi-check`; `make lint`
- [x] API-008 Implement `POST /v1/tool-calls` through the application service: authenticate, derive context, resolve immutable tool version, validate/normalize arguments, project resource, acquire logical invocation, authorize, persist required decisions, and execute only through a connector port. — completed 2026-09-02, evidence: `go test -race -count=1 ./internal/app ./internal/ports ./internal/adapters/http ./internal/adapters/postgres`; `go test -count=1 ./...`; `go vet ./...`; `make openapi-check`; `make lint`
- [x] API-009 Implement `GET /v1/tool-calls/{tool_call_id}` with tenant/run authorization, stable state/result representation, enumeration-safe behavior, and no credential/internal-provider leakage. — completed 2026-09-02, evidence: `go test -race -count=1 ./internal/app ./internal/ports ./internal/adapters/http ./internal/adapters/postgres`; `go test -count=1 ./...`; `go vet ./...`; `make openapi-check`; `make lint`
- [x] API-010 Implement a hermetic mock connector supporting deterministic reads, writes, delays, retry classes, injected transport errors, ambiguous outcomes, and reconciliation for application/integration testing. — completed 2026-09-02, evidence: `go test -race -count=1 ./internal/adapters/connectors/mock ./internal/ports`; `go test -count=1 ./...`; `go vet ./...`; `make lint`
- [x] API-011 Define and implement stable REST error codes for authentication, context, tool/version, schema, replay conflict, authorization, approval, GR, credential, connector, downstream rejection, ambiguity, result blocking, rate limiting, and budget exhaustion. — completed 2026-09-02, evidence: `go test -race -count=1 ./internal/adapters/http ./internal/domain ./internal/app`; `go test -count=1 ./...`; `go vet ./...`; `make openapi-check`; `make lint`
- [x] API-012 Add strict request/result size, timeout, content-type, pagination, concurrency, and schema-complexity limits to public handlers. — completed 2026-09-02, evidence: `go test -race -count=1 ./internal/adapters/http ./internal/schema ./internal/app`; `go test -count=1 ./...`; `go vet ./...`; `make lint`
- [x] API-013 Implement initial administrative publication/exposure endpoints only behind a separate privileged authorization path; do not allow ordinary harness credentials to mutate tool definitions or connector bindings. — completed 2026-09-03, evidence: `go test -race -count=1 ./internal/adapters/http ./internal/domain ./internal/ports`; `go test -count=1 ./...`; `go vet ./...`; `make openapi-check`; `make lint`
- [x] API-014 Run OpenAPI conformance, tenant isolation, replay/conflict, malformed argument, timeout, cancellation, and deterministic discovery tests. — completed 2026-09-03, evidence: `make test-phase4`; `go test -race -count=1 ./internal/adapters/http ./internal/app ./internal/adapters/postgres`; `go test -count=1 ./...`; `go vet ./...`; `make openapi-check`; `make lint`
- [x] API-015 Prove a governed mock invocation completes end to end through authentication, authorization, persistence, execution, evidence/outbox, and idempotent replay; record `docs/phase-4-evidence.md`. — completed 2026-09-03, evidence: `TestPhase4GovernedMockInvocationE2E` against PostgreSQL 18.6; `make test-phase4`; `go test -count=1 ./...`; `go test -race -count=1 ./internal/adapters/postgres ./internal/app ./internal/adapters/http ./internal/adapters/connectors/mock`; `go vet ./...`; `make openapi-check`; `make lint`
...
- [x] CRED-003 Implement typed credential capability objects with expiration, audience/resource, optional refresh/lease metadata, zeroizable/short-lived in-memory representation where practical, and secret-safe `String`/error behavior. — completed 2026-09-03, evidence: `go test -race -count=1 ./internal/credentials ./internal/ports ./internal/app`; `go test -count=1 ./...`; `go vet ./...`; `make lint`
- [x] CRED-004 Implement a development-only environment/file provider with explicit production prohibition, strong redaction, and isolated tests. — completed 2026-09-03, evidence: `go test -race -count=1 ./internal/adapters/credentials/development ./internal/credentials ./internal/ports`; `go test -count=1 ./...`; `go vet ./...`; `make lint`
- [x] CRED-005 Implement at least one production-oriented provider such as Kubernetes projected/workload identity, Vault, AWS STS, Google workload federation, Azure managed identity, or standards-based OAuth token exchange; document the selected RC provider. — completed 2026-09-03, evidence: `go test -race -count=1 ./internal/adapters/credentials/kubernetes ./internal/credentials ./internal/ports`; `go test -count=1 ./...`; `go vet ./...`; `make lint`; `docs/adr/0003-kubernetes-projected-credential-provider.md`
- [x] CRED-006 Implement bounded credential/token caching keyed only by trusted binding/context with expiry skew, revocation/rotation behavior, singleflight/concurrency safety, and no persistence of plaintext credentials. — completed 2026-09-03, evidence: `go test -race -count=1 ./internal/credentials ./internal/adapters/credentials/development ./internal/adapters/credentials/kubernetes ./internal/ports`; `go test -count=1 ./...`; `go vet ./...`; `make lint`
- [x] CRED-007 Implement a connector registry whose connector instances/destinations are administrator-controlled and immutable or versioned; ordinary tool arguments cannot select arbitrary hosts/connectors. — completed 2026-09-03, evidence: `go test -race -count=1 ./internal/domain ./internal/connectors ./internal/adapters/http`; `go test -count=1 ./...`; `go vet ./...`; `make lint`
- [x] CRED-008 Implement shared downstream HTTP security primitives: allowlisted schemes/hosts, TLS verification, bounded redirects, metadata/private-address protections as applicable, deadlines, body limits, safe headers, and telemetry. — completed 2026-09-03, evidence: `go test -race -count=1 ./internal/connectors/downstreamhttp ./internal/telemetry`; `go test -count=1 ./...`; `go vet ./...`; `make lint`; `make fmt-check`
- [x] CRED-009 Implement the first GitHub connector instance and read-only operation using a trusted credential binding; validate repository/resource projection before downstream execution. — completed 2026-09-03, evidence: `go test -race -count=1 ./internal/adapters/connectors/github`; `go test -count=1 ./...`; `go vet ./...`; `make lint`; `make fmt-check`
- [x] CRED-010 Implement `github.pull.comment` or equivalent reference write operation with documented side-effect/retry semantics and native downstream idempotency propagation where the API supports it. — completed 2026-09-03, evidence: `go test -race -count=1 ./internal/adapters/connectors/github`; `go test -count=1 ./...`; `go vet ./...`; `make lint`; `docs/connectors/github-pull-comment-qualification.md`
- [x] CRED-011 Record provider request/result identifiers and bounded safe metadata needed for reconciliation/evidence without storing bearer tokens or sensitive full payloads. — completed 2026-09-03, evidence: `go test -race -count=1 ./internal/adapters/connectors/github ./internal/app`; `go test -count=1 ./...`; `go vet ./...`; `make lint`; `make fmt-check`
- [x] CRED-012 Add connector conformance tests for deadline/cancellation, HTTP status/error mapping, response limits, redirect rejection, DNS/host policy, auth-header non-leakage, credential expiry, and server-side rate limiting. — completed 2026-09-03, evidence: `go test -race -count=1 ./internal/adapters/connectors/github ./internal/connectors/downstreamhttp ./internal/credentials`; `go test -count=1 ./...`; `go vet ./...`; `make lint`; `make fmt-check`
- [x] CRED-013 Add secret-canary tests across HTTP responses, application errors, logs, traces, metrics, audit/outbox payloads, GR requests, database rows, and panic paths. — completed 2026-09-03, evidence: `go test -race -count=1 ./internal/app ./internal/adapters/http ./internal/adapters/fakes ./internal/adapters/postgres ./internal/telemetry`; `go test -count=1 ./...`; `go vet ./...`; `make lint`
- [x] CRED-014 Run real or isolated-canary GitHub read/write qualification proving unauthorized calls never resolve credentials, credentials never reach the harness, and the reference write is correlated to a governed invocation. — completed 2026-09-03, evidence: `go test -race -count=1 ./internal/adapters/connectors/github`; `go test -count=1 ./...`; `go vet ./...`; `make lint`; `make fmt-check`; `docs/connectors/github-pull-comment-qualification.md`
- [x] CRED-015 Record connector/credential architecture and `docs/phase-5-evidence.md`; do not leave live provider tokens or captured payloads in the repository. — completed 2026-09-03, evidence: `docs/architecture/connectors-and-credentials.md`; `docs/phase-5-evidence.md`; `go test -race -count=1 ./internal/adapters/connectors/github ./internal/connectors/downstreamhttp ./internal/credentials ./internal/adapters/credentials/development ./internal/adapters/credentials/kubernetes`; `go test -count=1 ./...`; `go vet ./...`; `make lint`; `make fmt-check`
