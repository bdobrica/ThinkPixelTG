...
| TM-11 | MCP Origin, routing-header, continuation, or request-ID attacks | pinned revision, strict Origin/Host validation, stateless per-request identity/capability validation, header/body agreement, bounded continuation/cancellation, body limits; logical ID separate from JSON-RPC ID | conformance, cross-context replay, header mismatch, and cross-origin tests |
